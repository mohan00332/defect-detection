...
def _preprocess(frame):
    ...
    return cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR), edges
...
def detect_frame(frame, conf: float = None, iou: float = None, label_mode: str = "confidence", draw_counts: bool = False):
    ...
    model_input, edges = _preprocess(frame)
    if _use_filter is None:
        try:
            ...
            _use_filter = False
    if iou is None:
        results = model(model_input, conf=conf)
    else:
        results = model(model_input, conf=conf, iou=iou)
    ...
    return frame, detected, defect, edges
...
if __name__ == "__main__":
    ...
    while True:
        ...
        frame, _, _, edges = detect_frame(frame)
